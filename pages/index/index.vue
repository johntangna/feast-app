		...
		<block v-else>
			<view class="header">
				<view class="left">
					...
					<view v-if="distance" class="address" :style="'color:'+template.color.color3">距离您
						{{(distance/1000)>1?distance/1000+'千米':distance+'米'}}
					</view>
					<view v-else class="address" :style="'color:'+template.color.color3">{{shopInfo.shop_address}}
					</view>
				</view>
				<view class="right">
					<!-- <view class="type" :style="'background-color:'+template.color.color1">
						<view :class="template.overall.delivery_type===2?'name':'text'"
							:style="template.overall.delivery_type===2?'background-color:'+template.color.color1:''" @click="infoShow=!infoShow">预约
						</view>
					</view> -->
					<view class="more" @click="infoShow=!infoShow" :style="'color:'+template.color.color3">
						{{infoShow?'收起门店信息':'更多门店信息'}}
						<uni-icons :type="infoShow?'up':'down'" size="15" color="#999"></uni-icons>
					</view>
				</view>
			</view>
			<view style="height: 112px;"></view>
			<view class="goodsBox" :style="'height:'+goodsHeight+'px;'">
				<view class="left">
					<scroll-view scroll-y scroll-with-animation :style="'height:'+goodsHeight+'px;'" class="menuList">
						<view class="menu" :class="menuCurrent==index?'active':''"
							:style="menuCurrent==index?'color:'+template.color.color2+';border-color:'+template.color.color1:'color:'+template.color.color3"
							@click="selectMenuTap(index)" v-for="(item, index) in goodsAllList" :key="index">
							{{item.title}}
						</view>
				...
				<view class="right">
					<scroll-view scroll-y scroll-with-animation :scroll-into-view="goodsInto" @scroll="goodsScroll"
						:style="'height:'+goodsHeight+'px;'">
						<view v-if="swiperList.length>0" class="swiperBpx">
							<swiper class="swiper" circular autoplay interval="6000" duration="500"
								@change="swiperChange">
								<swiper-item v-for="(item, index) in swiperList" :key="index"
									@click="getBannerTap(item.target_data)">
						...
						<view id="goodsQuery" v-for="(item, index) in goodsAllList" :key="index">
							<view class="goodsList" :id="'gory-'+index">
								...
								<view class="goods lineBottom"
									v-for="(itm, idx) in item.goods_details" :key="idx"
									@click="getDteailsTap(index, idx)">
									<view class="left">
										<view class="image">
											<image :src="itm.pic" lazy-load mode="aspectFill" />
											<uni-icons v-if="itm.recom_status" class="icon" type="hot" size="30"
												color="#d3aa79"></uni-icons>
											<view v-if="itm.stock_number==0" class="none"
												:style="'color:'+template.color.color5">已售罄</view>
										</view>
									</view>
									<view class="right">
										<view class="title" :style="'color:'+template.color.color2">{{itm.title}}</view>
										<view class="intro" :style="'color:'+template.color.color3">{{itm.introduce}}
										</view>
										<view class="box">
											...
											<view class="button">
												<block v-if="openStoreStatus">
													<block v-if="itm.specs_status">
														<view v-if="itm.stock_number" class="text"
															:style="'background-color:'+template.color.color1">选规格
														</view>
														<view v-else class="text"
															:style="'background-color:'+template.color.color5">选规格
														</view>
													</block>
													<block v-else>
														<uni-icons v-if="itm.stock_number"
															@click.stop="addCartGoods(itm.id, 0, 1, 'add')" type="jia"
															size="23" :color="template.color.color1"></uni-icons>
														<uni-icons v-else type="jia" size="23"
															:color="template.color.color5"></uni-icons>
													</block>
												</block>
												<block v-else>
													<view v-if="itm.specs_status" class="text"
														:style="'background-color:'+template.color.color5">选规格</view>
													<uni-icons v-else type="jia" size="23"
														:color="template.color.color5">
													</uni-icons>
												</block>
											</view>
										</view>
			...
			<view v-if="openStoreStatus && cartList.length>0" class="footer" :class="IphoneX>=0?'footerX':''">
				<view class="left">
					<view class="cart" @click="cartShow=!cartShow" :style="'color:'+template.color.color2">
						<uni-icons type="liwu" size="18" color="#343434"></uni-icons>预约单<text
							:style="'background-color:'+template.color.color1">{{cartNumber}}</text>
					</view>
					<view class="price" :style="'color:'+template.color.color2">¥{{cartTotal}}</view>
				</view>
				<view class="right">
					<block v-if="template.overall.delivery_type===2">
						<view v-if="cartTotal>0" class="button" @click="toPaypalTap()"
							:style="'background-color:'+template.color.color1">前去干饭</view>
						<view v-else class="button none" :style="'background-color:'+template.color.color5">
							自助取货
						</view>
					</block>
				</view>
			...
			<!-- <view v-if="!openStoreStatus" class="footer" :class="IphoneX>=0?'footerX':''" style="overflow: hidden;">
				<view class="label" :style="'background-color:'+template.color.color2">本店已休息，营业时间：{{openStoreTime}}
				</view>
			</view> -->
			...
			<view v-if="goodShow" class="showPopBox" style="display: flex;align-items: center;justify-content: center;">
				...
				<view class="goodsConten">
					<view class="image">
						...
						<uni-icons @click="goodShow=false" type="close" size="25" :color="template.color.color5"
							style="position:absolute;top: 10rpx; right: 10rpx;"></uni-icons>
						<!-- <uni-icons @click="shareShow=true" type="fenxiang" size="25" :color="template.color.color1"
							style="position:absolute;top: 10rpx; left: 10rpx;"></uni-icons> -->
						<uni-icons @click="favGoodsTap(goodsOneData.id)" type="shoucang" size="25"
							:color="template.color.color1" style="position:absolute;top: 10rpx; left: 10rpx;">
						</uni-icons>
					</view>
					...
					<scroll-view scroll-y class="moreBox" style="max-height: 500rpx;">
						<view class="more">
							...
							<view class="intro" :style="'color:'+template.color.color3">{{goodsOneData.introduce}}
							</view>
						</view>
					</scroll-view>
					<view class="gfooter">
						<view class="price">
							<view class="left">
								<view v-if="childProductData.child_product_price" class="text"
									:style="'color:'+template.color.color1">
									¥{{childProductData.child_product_price}}<text
										:style="'color:'+template.color.color3"
										v-if="childProductData.child_product_cost">¥{{childProductData.child_product_cost}}</text>
								</view>
								<view v-else class="text" :style="'color:'+template.color.color1">
									¥{{goodsOneData.mini_price}}<text :style="'color:'+template.color.color3"
										v-if="goodsOneData.max_price">¥{{goodsOneData.max_price}}</text></view>
								<view class="label" v-if="childProductData.child_product_spec"
									:style="'color:'+template.color.color3">
									<text v-for="(value, name) in childProductData.child_product_spec"
										:key="name">{{name}}:{{value}} </text>
								</view>
							</view>
							<view class="right">
								<view class="buy">
									<uni-icons type="jian" @click="editGoodsOne('jian')" size="23"
										:color="buyNumber<=1?template.color.color5:template.color.color1"></uni-icons>
									<input type="number" :value="buyNumber" disabled
										:style="'color:'+template.color.color2" />
									<uni-icons type="jia" @click="editGoodsOne('jia')" size="23"
										:color="buyNumber>=goodsOneData.stock_number?template.color.color5:template.color.color1">
									</uni-icons>
								</view>
							</view>
						</view>
						<block>
							<block>
								<view v-if="goodsOneData.specs_status && !childProductData" class="button none"
									:style="'background-color:'+template.color.color5">加入预约单</view>
								<view v-if="goodsOneData.specs_status && childProductData" class="button"
									@click="addShopCar()" :style="'background-color:'+template.color.color1">加入预约单
								</view>
								<view v-if="!goodsOneData.specs_status" class="button" @click="addShopCar()"
									:style="'background-color:'+template.color.color1">加入预约单</view>
							</block>
						</block>
					</view>
				</view>
					...
					<view class="sheade">
						...
						<view class="right">
							<view class="text" @click="delNoneCartTap()">
								<uni-icons type="shanchu" size="14"></uni-icons> 清空预约单
							</view>
						</view>
					</view>
					<view class="sconten" :style="IphoneX>=0?'padding-bottom:146rpx;':'padding-bottom: 120rpx;'">
						<view class="goods" v-for="(item, index) in cartList" :key="index"
							:style="cartList.length==(index+1)?'border-bottom: none;':''">
							<view class="left">
							...
							<view class="right">
								...
								<view class="buy">
									<block v-if="item.shelve_status">
										<block>
											<uni-icons type="jian" @click="editGoods(item, 'jian')" size="23"
												:color="item.num==1?'#cccccc':'#d3aa79'"></uni-icons>
											<input type="number" :value="item.num" disabled />
											<uni-icons type="jia" @click="editGoods(item, 'jia')" size="23"
												:color="item.num>=item.stock_number?'#cccccc':'#d3aa79'"></uni-icons>
										</block>
									</block>
									<block v-else>
										<text>商品已下架</text>
			...
			<view v-if="infoShow" class="showPopBox" style="z-index: 899;">
				...
				<view class="infoContent">
					...
					<view v-if="template.overall.licence_list.length>0" class="label">
						...
						<view class="images">
							<view class="image" v-for="(item, index) in template.overall.licence_list" :key="index"
								@click="previewImage(index)">
								<image mode="aspectFill" :src="item.pics" lazy-load></image>
								<view v-if="item.title" class="text" :style="'color:'+template.color.color3">
									{{item.title}}
								</view>
							</view>
						</view>
			...
			<view v-if="shareShow" class="showShare">
				...
				<view class="share">
					...
					<view v-if="haibaoStu" class="haibao">
						...
						<view class="save" @click="saveImageToPhotosAlbum()"
							:style="'background-color:'+template.color.color1">保存海报到本地</view>
					</view>
					<view v-else class="box">
						<view class="label">
							<uni-icons type="weixin" size="42" color="#07c160"></uni-icons>
						...
						<view class="label" @click="getGoodsHaibao()">
							<uni-icons type="haibao" size="42" :color="template.color.color1"></uni-icons>
			...
			<view v-if="couponsList.length>0 && !couponsShow" @click="couponsShow=true" class="couponsIcon"
				:style="cartList.length>0?'bottom: 250rpx;':'bottom: 160rpx;'">
				<image v-if="template.overall.coupons_icon" mode="aspectFill" lazy-load
					:src="template.overall.coupons_icon"></image>
			...
			<view v-if="couponsShow" class="showCoupons">
				...
				<view class="coupons">
					...
					<scroll-view scroll-y class="listBox" style="height:246rpx;">
						<view class="list" v-for="(item, index) in couponsList" :key="index">
							...
							<view class="right">
								...
								<view class="times" v-if="item.time_type===1" :style="'color:'+template.color.color3">
									{{item.end_time}}日截止
								</view>
								<view class="times" v-else :style="'color:'+template.color.color3">
									领取后{{item.time_day}}天内有效</view>
							</view>
						</view>
					</scroll-view>
					...
					<uni-icons type="guanbi" color="#fff" size="35" class="close" @click="couponsShow=false">
					</uni-icons>
				</view>
			</view>
		</block>
...
<script>
	...
	import QQMapWX from '@/static/utils/h5map.min.js'
	import Request from 'static/utils/request.js';
	import GoodsDetails from 'static/utils/goodDetails.js';
	const app = getApp();
	export default {
		components: {
			UniIcons
				...
				return (goodsArr) => {
					...
					for (let i = 0; i < goodsArr.length - 1; i++) {
						for (let j = i; j > 0; j--) {
							...
							let needCompare = goodsArr[j - 1].goods_id._value
							if (needCompare > _value) {
								temp = goodsArr[j - 1]
		...
		methods: {
			...
			getCouponsList() {
				...
				if (self.openStoreStatus && self.template.overall.coupons_list) {
					self.Post(self.Url.couponsGoods, {
						coupons_list: self.template.overall.coupons_list
					}).then(res => {
						if (res.code === 0) {
							let list = [];
				...
				if (temp.overall.qqmap_key && temp.overall.latitude && temp.overall.longitude) {
					uni.getLocation({
						...
						success: function(gps) {
							self.mapCTX = new QQMapWX({
								key: temp.overall.qqmap_key
							});
			...
			getCartList() {
				...
				self.cartList.forEach(item => {
					number += item.num;
					totals += item.mini_price * item.num;
				})
			...
			async getGoodsAllList() {
				...
				} = await Request.baseQuery("bigmouth_goods", "goods_id")
				console.info(data)
				// data.forEach(async (item) => {
				// 	const res = await Request.uninoSelect("bigmouth_goods", "bigmouth_goods_details",
				// 		"goods_id", item
				// 		.goods_id, "title", "goods_id")
				// 	goodsArr.push(res.result.data[0])
				// })
				const res = await GoodsDetails.getAllGoods("bigmouth_goods_details", "fk_goods_id")
				console.info(res)
				const goodDetails = res.result.data
				
				data.forEach((item, index) => {
					item.goods_details = []
					goodDetails.forEach((details, detailsIndex) => {
						if(details.fk_goods_id == item.goods_id){
							item.goods_details.push(goodDetails[detailsIndex])
						}
					})
				})
				...
				// 	success: (res) => {
				self.loading = false;
				self.goodsAllList = data;
				app.globalData.goodsAllList = data
				setTimeout(function() {
					self.goodsQuery();
				}, 500)
				// 	}
			...
			getDteailsTap(index, idx) {
				let self = this;
				let data = self.goodsAllList[index].goods_details[idx];
				if (app.globalData.user) {
					self.childProductData = '';
			...
			selectSpecsTap(index, idx) {
				...
				self.$set(self.goodsOneData.specs_list.specification, index, self.goodsOneData.specs_list.specification[
					index])
				let curSelectNum = 0;
				...
				for (let i = 0; i < childProductArray.length; i++) {
					if (!self.canSubmit && Object.keys(selectSpecification).every(key => childProductArray[i]
							.child_product_spec[key] === selectSpecification[key])) {
			...
			addShopCar() {
				...
				if (data.specs_status) {
					...
					self.addCartGoods(data, 0, self.buyNumber, 'add');
				}
			},
			addCartGoods(data, spec, number, type) {
				if(number < 1) return
				let self = this;
				if (app.globalData.user) {
					let postData = {
						id: data.goods_details_id,
						number: number
					}
					let url = {};
					if (spec) {
					...
					self.goodShow = false;
					const exist = this.cartList.filter(cb => cb.id == data.goods_details_id)
					if(exist.length){
						this.cartList.forEach(item => {
							if(item.id == data.goods_details_id) {
								item.num = number
							}
						})
					} else {
			...
			editGoods(data, type) {
				let self = this;
				if (type == 'jian') {
					self.addCartGoods(data, 0, data.num - 1, 'edit');
				} else if (type == 'jia') {
						...
						wx.downloadFile({
							...
							success(qrcode) {
								...
								wx.downloadFile({
									url: pics,
									success(pic) {
										let sics = self.shopInfo.shop_logo.replace(/http:/g,
											'https:');
										wx.downloadFile({
											url: sics,
											success(sic) {
												let ctxs = wx.createCanvasContext(
													'goodsHaibao');
												let postData = {
												...
												if (data.title.split('').length >= 16) {
													postData.title = data.title.substr(0,
														16) + '...';
												} else {
													postData.title = data.title;
												}
												//商品介绍
												if (data.introduce.split('').length >=
													18) {
												...
												ctxs.drawImage(postData.shop_logo, 30, 30,
													60, 60)
												ctxs.restore()
												ctxs.save()
												...
												ctxs.drawImage(postData.pic, 30, 120, 540,
													540)
												ctxs.setFillStyle(postData.color.color1)
												...
												const minrics = ctxs.measureText(postData
													.mini_price)
												ctxs.setFontSize(30)
												ctxs.setTextAlign('left')
												ctxs.fillText(postData.max_price, 55 +
													minrics.width, 703)
												const maxrics = ctxs.measureText(postData
													.max_price)
												ctxs.beginPath()
												ctxs.setStrokeStyle('#FFFFFF')
												ctxs.setLineWidth(2)
												ctxs.moveTo(55 + minrics.width, 695)
												ctxs.lineTo(55 + minrics.width + maxrics
													.width, 695)
												ctxs.setFontSize(24)
												ctxs.setTextAlign('right')
												ctxs.fillText(postData.stock_number, 550,
													700)
												ctxs.stroke()
												ctxs.restore()
												...
												ctxs.drawImage(postData.qrcode, 430, 850,
													120, 120)
												ctxs.setFillStyle(postData.color.color5)
												ctxs.setFontSize(22)
												ctxs.setTextAlign('right')
												ctxs.fillText('长按扫描二维码查看详情', 400, postData
													.nikename ? 940 : 920)
												ctxs.fillText('来自' + postData.nikename +
													'的推荐', 400, 910)
												ctxs.restore()
												ctxs.save()
												ctxs.draw()
