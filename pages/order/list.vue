	...
	<view class="content">
		...
		<block v-else>
			<view v-if="orderList.length>0" class="orderListBox">
				<view class="list" v-for="(item, index) in orderList" :key="index" @click="getOrderDetail(item.order_id)">
					<view class="oHeader lineBottom">
						...
						<view class="right" :style="'color:'+template.color.color3">
							...
							<text v-if="item.order_status==1 && item.delivery_type==1">待自取</text>
					...
					<view class="goodsBox lineBottom">
						<view class="goods" v-for="(itm, idx) in item.order_details" :key="idx">
							<view class="left">
								<view class="image"><image lazy-load mode="aspectFill" :src="itm.pic" /></view>
							</view>
							<view class="right">
								...
								<view class="box">
									...
									<view class="number" :style="'color:'+template.color.color3">x{{itm.num}}</view>
								</view>
							</view>
						</view>
					</view>
					<view class="orderBox lineBottom">
						<view class="left">
							<view :style="'color:'+template.color.color3">预约时间：{{item.create_time}}</view>
							<view :style="'color:'+template.color.color3">预约单号：{{item._id}}</view>
						</view>
						<view class="right">
					...
					<view class="paypalList">
						<view class="paypal lineBottom" v-for="(item, index) in shopData.shop_paypal" :key="index" @click="getPaypalOrder(item.lid, item.pdata)">
							<view class="left">
								...
								<uni-icons v-if="item.lid==3" type="yuezhifu" size="30" :color="template.color.color1"></uni-icons>
								<uni-icons v-if="item.lid==5" type="jifen1" size="35" :color="template.color.color1"></uni-icons>
							</view>
							<view class="right">
							...
							<view class="icon">
								<uni-icons type="right" size="18" color="#e2e2e2"></uni-icons>
							</view>
						</view>
					</view>
				</view>
	...
	export default {
		...
		methods: {
			...
			async getOrderList() {
				...
				const {result: {data}} = await Order.allOrder("bigmouth_order", app.globalData.user_number)
				self.loading = false;
				self.orderList = data;
				self.orderList.forEach(item => {
					let totalMoney = 0
					item.order_details.forEach(it => {
						totalMoney += Number(it.total)
					})
