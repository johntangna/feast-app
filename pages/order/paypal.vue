	...
	<view class="content">
		<view class="shopInfo">
			<view class="left">
				...
				<view v-if="distance" class="address" :style="'color:'+template.color.color3">距离您
					{{(distance/1000)>1?distance/1000+'千米':distance+'米'}}，请自助前往取货
				</view>
		...
		<view class="delivery">
			<view class="selectType">
				...
				<view class="label"
					:style="deliveryNum===1?'border-color:'+template.color.color1+';background-color:'+template.color.color1+';color:#fff;flex: 0 0 100%;':'color:'+template.color.color2">
					<uni-icons type="mendian" size="20" :color="deliveryNum===1?'#fff':template.color.color2">
					</uni-icons>
					<text>门店自提</text>
				</view>
			</view>
			...
			<view v-if="deliveryNum==2" class="address">
				...
				<view class="input">
					<input type="number" v-model="userPhone" placeholder="请输入联系电话"
						:style="'color:'+template.color.color2" />
				</view>
				<view class="button"
					:style="'color:'+template.color.color1+';border-color:'+template.color.color1+'50'">
						...
						<view class="right">
							...
							<view class="box">
								<view class="price" :style="'color:'+template.color.color1">
									¥<text>{{item.mini_price}}</text></view>
				...
				<view v-if="couponsList.length>1" class="listBox lineBottom">
					...
					<view class="right">
						<picker @change="selectCoupons" :value="couponsIndex" range-key="title" :range="couponsList">
							<view class="name" :style="'color:'+template.color.color3" v-if="couponsIndex==0">
								共{{couponsList.length-1}}张优惠券可用</view>
							<view class="name" :style="'color:'+template.color.color2" v-else>
								{{couponsList[couponsIndex].title}}<text
									:style="'color:'+template.color.color5">(满¥{{couponsList[couponsIndex].min_money}}减¥{{couponsList[couponsIndex].moneys}})</text>
							</view>
						</picker>
					</view>
				</view>
		...
		<view class="orderBox">
			...
			<view class="remark lineBottom">
				...
				<view class="right">
					<input type="text" v-model="remarkText" placeholder="如有需要请备注说明" style="text-align: right;"
						:style="'color:'+template.color.color2" />
				</view>
			</view>
			<view class="order">
				<view class="label lineBottom">
					<view class="title" :style="'color:'+template.color.color2">商品合计<text
							:style="'color:'+template.color.color3">(共{{orderData.goods_number}}件商品)</text></view>
				...
				<view v-if="couponsIndex>0 && orderData.coupons_money>0" class="label lineBottom">
					<view class="title" :style="'color:'+template.color.color2">优惠金额<text
							:style="'color:'+template.color.color5">(满¥{{couponsList[couponsIndex].min_money}}减¥{{couponsList[couponsIndex].moneys}})</text>
					</view>
				...
				<view v-if="orderData.user_level" class="label lineBottom">
					<view class="title" :style="'color:'+template.color.color2">会员折扣<text
							:style="'color:'+template.color.color5">({{orderData.user_level}}{{orderData.user_count*10}}折)</text>
					</view>
		...
		<view class="footer" :class="IphoneX>=0?'footerX':''">
			...
			<view class="right">
				<block>
					<block>
						<view class="button" :style="'background-color:'+template.color.color1"
							@click="paypalOrderTap('form')">提交干饭魂
						</view>
					</block>
				</block>
			</view>
		...
		<view class="showContent" :style="paypalShow?'bottom:0;':''">
			...
			<view class="showBox">
				...
				<view class="payTime" :style="'color:'+template.color.color3">
					剩余支付时间：<countdown v-if="orderData.order_time_stamp" :targetTime="orderData.order_time_stamp">
					</countdown>
				</view>
				<view class="paypalList">
					<view class="paypal lineBottom" v-for="(item, index) in shopData.shop_paypal" v-if="item.lid!=5"
						:key="index" @click="getPaypalOrder(item.lid)">
						<view class="left">
...
<script>
	...
	export default {
		...
		onLoad() {
			...
			if (cart) {
				...
				self.orderData = {
					...
					goods_money: goods_money.toFixed(2),
		...
		methods: {
			...
			async confirmOrder(update = false) {
				let self = this;
				let temp = self.template;
				...
				if (app.globalData.user) {
					...
					this.shopCartList.forEach(item => {
						const tempObj = {
							...
							total: item.num * item.mini_price
						}
						order_details.push(tempObj)
					})
					const {
						result
					} = await Order.addOrder("bigmouth_order", app.globalData.user_number, this.remarkText,
						order_details, update)
					if (result.errCode == 0) {
						uni.hideLoading();
						uni.showModal({
							content: '已收到干饭人的干饭魂，正催促掌勺大厨',
							showCancel: false,
							success() {
								uni.reLaunch({
									url: '/pages/tabbar'
			...
			async paypalOrderTap(e) {
				...
				const one = await Request.getUcDb().collection("bigmouth_order").where(Request.dbCmd().and({
					create_time: Request.dateFormat("YYYY-mm-dd", new Date()),
					fk_user_number: Number(app.globalData.user_number)
				})).get({
					getOne: true
				})
				console.info(one)
