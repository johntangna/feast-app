				...
				<view class="gfooter">
					...
					<block v-if="openStoreStatus">
						<block v-if="buyActive">
							<view v-if="goodsOneData.specs_status && !childProductData" class="button none" :style="'background-color:'+template.color.color5">加入预约单</view>
							<view v-if="goodsOneData.specs_status && childProductData" class="button" @click="addShopCar()" :style="'background-color:'+template.color.color1">加入预约单</view>
							<view v-if="!goodsOneData.specs_status" class="button" @click="addShopCar()" :style="'background-color:'+template.color.color1">加入预约单</view>
						</block>
						<view v-else class="button none" :style="'background-color:'+template.color.color5">{{goodsOneData.stock_number===0?'商品已售罄':'加入预约单'}}</view>
					</block>
					<block v-else>
